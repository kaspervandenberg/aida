...
Document[] doc = result.getDocArray();
	    ...
	    FieldType[] fields = doc[counter].getFieldArray();
	    char[] inputChars = new char[10000];
	    
	    for(int ind = 0; ind < fields.length; ++ind){
	      if (fields[ind].getName().equalsIgnoreCase("content")){	
            	inputChars = doc[counter].getFieldArray(ind).getValue().toCharArray();
               }
	      }
	      firstFilter.startDocument();
              firstFilter.startSimpleElement(PLAIN_TEXT_TOP_LEVEL_ELEMENT);
