...
//alert("sendRequest Params: " + arguments[i]);
}
      var oThis = this;
...
//alert("injectSuggest for:" + this.id);
      if ( this.isIE )
...
//alert("rico ajax");
      ...
      callParms.push( 'index='          + this.options.index);
      callParms.push( 'field='          + this.options.field);
      callParms.push( 'property='       + this.lastRequestString );
      ...
      for( var i=0 ; i < callParms.length ; i++ )
//alert("rico ajax Param: " + callParms[i]);

//alert("rico ajax, ajaxEngine: " + ajaxEngine); 
...
//alert("end rico ajax");
   ...
   createSuggestions: function(ajaxResponse) {
      ...
      for ( var i = 0 ; i < entries.length ; i++ ) {
         ...
         var strComment = this.getElementContent(entries[i].getElementsByTagName('comment')[0]);
         this.suggestions.push( { text: strText, value: strValue, comment: strComment } );
      }
   },
...
//alert("position suggest div: "+divStyle.left+","+divStyle.top);
...
//alert("adding div "+suggestLines[i]);      
      }	 
   },
...
//alert("suggestions: "+this.suggestions.length);
   ...
   createSuggestionSpan: function( n, regExp ) {
      ...
      commentSpan.id            = this.id + "_comment_" + n;
      commentSpan.className     = "commentSpan";
      commentSpan.onmouseover   = this.mouseoverHandler.bindAsEventListener(this);
      commentSpan.onclick       = this.itemClickHandler.bindAsEventListener(this);
            
      commentSpan.appendChild( document.createTextNode( suggestion.comment ) );
      suggestionSpan.appendChild( commentSpan );
