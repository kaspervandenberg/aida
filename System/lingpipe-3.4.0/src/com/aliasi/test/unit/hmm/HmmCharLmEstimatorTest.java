...
public class HmmCharLmEstimatorTest extends BaseTestCase {
	...
	new HmmCharLmEstimator(5,256,4.0);
	succeed();
	assertIllegalParams(0,256,4.0);
	assertIllegalParams(-1,256,4.0);
	assertIllegalParams(2,0,4.0);
	assertIllegalParams(2,-1,4.0);
	assertIllegalParams(2,1000000,4.0);
	assertIllegalParams(2,256,-1);
    }
    
    void assertIllegalParams(int a, int b, double c) {
	try {
	    new HmmCharLmEstimator(a,b,c);
	    fail();
	...
	est.handle(toks,null,tags1);
	est.handle(toks,null,tags2);
	est.handle(toks,null,tags3);
	est.handle(toks,null,tags4);
