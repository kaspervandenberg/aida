...
public class StatisticsTest extends BaseTestCase {
    ...
    public void testPermutation() {
        ...
        for (int i = 0; i < xs.length; ++i) {
            ...
            xSet.add(new Integer(xs[i]));
        }
        assertEquals(100,xSet.size());
    }

    public void testRegressionExceptions() {
        double[] xs = { 1 };
        double[] ys = { 2 };
        try {
    ...
    public void testChiSquareMatrix() {
        ...
        try {
            matrix = new double[][] { { 1, 2, 3}, {4, Double.NaN, 6} };
            Statistics.chiSquaredIndependence(matrix);
            fail();
        } catch (IllegalArgumentException e) {
            succeed();
        ...
        try {
            matrix = new double[][] { { 1, 2, 3}, {4, 5, Double.POSITIVE_INFINITY} };
            Statistics.chiSquaredIndependence(matrix);
            fail();
        } catch (IllegalArgumentException e) {
            succeed();
    ...
    public void testDirichlet() {
        ...
        double log2Expected2 = com.aliasi.util.Math.log2Gamma(3 + 4 + 5)
            ...
            + com.aliasi.util.Math.log2(Math.pow(0.2,3-1))
            + com.aliasi.util.Math.log2(Math.pow(0.3,4-1))
            + com.aliasi.util.Math.log2(Math.pow(0.5,5-1));

        assertDirichlet(new double[] { 3, 4, 5 },
                        new double[] { 0.2, 0.3, 0.5 },
                        Math.pow(2.0, log2Expected2));
